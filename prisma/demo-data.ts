/**
 * Idempotent demo-data populator.
 *
 * Run: npm run db:demo
 *
 * Skips inserts if the IMEI / serial / part name already exists, so it's
 * safe to re-run and safe to run against a production DB that already
 * has real records.
 */
...
const PHONES: DemoPhone[] = [
  ...
  { brand: "Apple", model: "iPhone 13 Pro", storage: "256GB", color: "Sierra Blue", condition: "Like New", imei: "DEMO-356001000000008", serial: "DEMO-FK7C008", purchasePrice: 480, askingPrice: 699, status: "for_sale", purchaseDaysAgo: 25, city: "gta", notes: "New OEM screen installed last month.", repairNeeded: "" },
...
const PARTS: DemoPart[] = [
  // High-end OEM screens for headline models
...
async function main() {
  // Ensure singleton Settings row exists (additive; won't overwrite)
  ...
  // Ensure a "Demo Wholesaler" supplier exists
  let supplier = await prisma.supplier.findFirst({ where: { name: "Demo Wholesaler" } });
  if (!supplier) {
    supplier = await prisma.supplier.create({
      data: { name: "Demo Wholesaler", contact: "demo@example.com", notes: "Auto-created by db:demo script. Safe to delete." }
    });
    console.log("• Created Demo Wholesaler supplier");
  }

  // Phones — idempotent on IMEI
  let phoneAdded = 0;
  let phoneSkipped = 0;
  ...
  // Parts — idempotent on (name + compatibleWith)
  let partAdded = 0;
  let partSkipped = 0;
  ...
  console.log(`\n✓ Demo data loaded`);
  console.log(`  Phones — added ${phoneAdded}, skipped ${phoneSkipped} already present`);
  console.log(`  Parts  — added ${partAdded}, skipped ${partSkipped} already present`);
  console.log(`\nAll demo records carry the prefix "DEMO-" on IMEI / serial / supplier name.`);
  console.log(`To remove later: filter Phone.imei startsWith "DEMO-", Part by name match, and the "Demo Wholesaler" supplier.`);
}
