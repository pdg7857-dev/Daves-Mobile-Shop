...
export default async function OrderStatusPage({
  ...
  if (!sp.email) {
    return (
      <div className="mx-auto max-w-xl px-4 sm:px-6 lg:px-8 py-12">
        ...
        <p className="mt-2 text-sm text-gray-400">Enter the email you used at checkout to view order {orderNumber}.</p>
  ...
  return (
    <div className="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8 py-10">
      <Link href="/orders" className="text-sm text-brand-300 hover:text-brand-200">← Look up another order</Link>

      {isNew && order.status === "pending_payment" && (
        <div className="mt-4 card p-5 bg-green-900/30 border-green-700/50">
          <h2 className="text-lg font-semibold text-green-300">🎉 Order placed!</h2>
      ...
      <div className="mt-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-gray-400">Placed {date(order.createdAt)}</p>
        </div>
      ...
      {order.trackingNumber && (
        <div className="mt-4 card p-4 bg-purple-900/30 border-purple-700/50">
          <h3 className="font-semibold text-purple-300">📦 Shipped</h3>
      ...
      <div className="mt-6 card overflow-hidden">
        ...
        <table className="w-full text-sm">
          <thead className="bg-gray-900/40 text-left text-xs uppercase tracking-wide text-gray-400">
            <tr>
        ...
        <dl className="border-t border-gray-800 p-4 space-y-1 text-sm">
          <div className="flex justify-between"><dt className="text-gray-400">Subtotal</dt><dd>{money(order.subtotal)}</dd></div>
          ...
          <div className="flex justify-between"><dt className="text-gray-400">Shipping</dt><dd>{order.shippingCost === 0 ? "Free" : money(order.shippingCost)}</dd></div>
          <div className="flex justify-between"><dt className="text-gray-400">Tax{province ? ` (${province.taxLabel})` : ""}</dt><dd>{money(order.taxAmount)}</dd></div>
          <div className="flex justify-between text-base font-bold border-t border-gray-800 pt-2 mt-2">
            <dt>Total</dt><dd className="text-brand-300">{money(order.total)}</dd>
          </div>
        </dl>
      </div>
      ...
      <div className="mt-6 grid sm:grid-cols-2 gap-4">
        <div className="card p-5">
          ...
          <address className="mt-2 not-italic text-sm text-gray-300 leading-relaxed">
        ...
        <div className="card p-5">
          ...
          <p className="mt-2 text-sm text-gray-300">{order.customerEmail}</p>
          {order.customerPhone && <p className="text-sm text-gray-300">{order.customerPhone}</p>}
          {order.customerNotes && (
      ...
      {order.daveCarePlans.length > 0 && (
        <div className="mt-8">
          <h2 className="text-2xl font-bold text-white">🛡️ Your Dave Care coverage</h2>
          <p className="mt-1 text-sm text-gray-400">Bring your device in for any of these and we&apos;ll take care of it.</p>
          <div className="mt-4 space-y-3">
            {order.daveCarePlans.map((p) => (
              <div key={p.id} className="card p-5">
                <div className="flex items-center justify-between gap-3 flex-wrap">
                  <div>
                    <div className="font-semibold text-white">{p.phoneLabel}</div>
                ...
                <ul className="mt-3 grid sm:grid-cols-2 gap-2 text-sm">
                  {DAVE_CARE_BENEFITS.map((b) => {
                    const used = p[b.flag];
                    return (
                      <li key={b.key} className={`flex items-center gap-2 ${used ? "text-gray-500 line-through" : "text-gray-200"}`}>
                        <span>{used ? "✗" : "✓"}</span>
                        <span>{b.label}{used ? " (used)" : ""}</span>
                      </li>
                    );
                  })}
                </ul>
                {p.expiresAt && (
                  <p className="mt-3 text-xs text-gray-500">Coverage through {date(p.expiresAt)}</p>
                )}
              </div>
            ))}
          </div>
