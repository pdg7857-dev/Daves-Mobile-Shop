...
export default async function DashboardPage() {
  const [
    ...
    pendingOrders, paidOrders, lowStock, recentIntake, recentOrders
  ] = await Promise.all([
    ...
    <div>
      ...
      <div className="mt-6 grid sm:grid-cols-2 gap-4">
        <div className="card p-4">
          ...
          <div className="mt-2 text-3xl font-bold text-brand-700">{money(inventoryValue._sum.askingPrice ?? 0)}</div>
        </div>
        <div className="card p-4">
          <h2 className="font-semibold text-gray-900">Order revenue (paid+)</h2>
          <div className="mt-2 text-3xl font-bold text-green-700">{money(ordersRevenue._sum.total ?? 0)}</div>
        </div>
      </div>
      ...
      <div className="mt-6 grid lg:grid-cols-2 gap-6">
        <div className="card p-4">
          <div className="flex items-center justify-between">
            ...
            <Link href="/admin/orders" className="text-sm text-brand-700">All →</Link>
          </div>
          <ul className="mt-3 divide-y divide-gray-100">
              ...
              return (
                <li key={o.id} className="py-2 flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <Link href={`/admin/orders/${o.id}`} className="font-mono text-xs text-brand-700 hover:text-brand-900">
                      {o.orderNumber}
                    </Link>
                    <div className="text-xs text-gray-500 truncate">
                      {o.customerName} · {o.city}, {o.province} · {date(o.createdAt)}
                    </div>
                  </div>
            ...
            {recentOrders.length === 0 && (
              <li className="py-4 text-sm text-gray-500">No orders yet.</li>
            )}
          </ul>
        </div>
      ...
      <div className="mt-6 card p-4">
        <div className="flex items-center justify-between">
          <h2 className="font-semibold text-gray-900">Low-stock parts ({partsCount} total SKUs)</h2>
