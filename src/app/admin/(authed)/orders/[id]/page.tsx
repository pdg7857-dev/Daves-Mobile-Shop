      ...
      <div className="mt-6 grid lg:grid-cols-3 gap-6">
                ...
                {order.items.map((i) => (
                  <tr key={i.id}>
                    ...
                    <td className="table-cell text-xs">
                      {i.phone && (
                        <Link href={`/admin/inventory/${i.phone.id}`} className="text-brand-700 hover:text-brand-900">
                          📱 Phone · IMEI {i.phone.imei || "—"}
                        </Link>
                      )}
                      {i.part && (
                        <Link href={`/admin/parts/${i.part.id}`} className="text-brand-700 hover:text-brand-900">
                          🔧 Part · stock now {i.part.stock}
                        </Link>
                      )}
                    </td>
        ...
        <aside className="space-y-5">
          <div className="card p-5">
            ...
            <p className="text-sm">
              <a href={`mailto:${order.customerEmail}`} className="text-brand-700 hover:text-brand-900">{order.customerEmail}</a>
            </p>
            {order.customerPhone && (
              <p className="text-sm">
                <a href={`tel:${order.customerPhone}`} className="text-brand-700 hover:text-brand-900">{order.customerPhone}</a>
              </p>
            )}
          </div>
          ...
          {order.customerNotes && (
            <div className="card p-5 bg-amber-50 border-amber-100">
              <h3 className="font-semibold text-amber-900">Customer notes</h3>
