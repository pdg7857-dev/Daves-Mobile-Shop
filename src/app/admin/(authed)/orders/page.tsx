  ...
  return (
    <div>
      ...
      <div className="mt-6 flex flex-wrap gap-2">
        <Link href="/admin/orders" className={`text-sm rounded-full px-3 py-1 border ${!sp.status ? "bg-brand-700 text-white border-brand-700" : "bg-white text-gray-700 border-gray-300"}`}>All ({orders.length})</Link>
        {(["pending_payment", "paid", "processing", "shipped", "delivered", "cancelled"] as OrderStatus[]).map((s) => (
          <Link key={s} href={`/admin/orders?status=${s}`} className={`text-sm rounded-full px-3 py-1 border ${sp.status === s ? "bg-brand-700 text-white border-brand-700" : "bg-white text-gray-700 border-gray-300"}`}>{ORDER_STATUS_LABELS[s]} ({countMap[s] || 0})</Link>
        ))}
      </div>
