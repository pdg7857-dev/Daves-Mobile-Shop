  ...
  return (
    <div className="mt-6 grid lg:grid-cols-3 gap-8">
                ...
                {item.daveCarePlan ? (
                  <div className="flex items-center justify-between gap-3">
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <div>
                        <span className="font-medium text-gray-900">Dave Care</span>
                        <span className="ml-2 text-xs text-gray-500">{item.daveCarePlan === "monthly" ? `${money(DAVE_CARE_PRICES.monthly)}/mo` : `${money(DAVE_CARE_PRICES.annual)}/yr`}</span>
                      </div>
                    </div>
                    <div className="flex items-center gap-3">
                      <span className="text-sm font-medium text-brand-700">+{money(DAVE_CARE_PRICES[item.daveCarePlan])}</span>
                      <button onClick={() => pickPlan(item.id, null)} className="text-xs text-red-700 hover:text-red-900">Remove</button>
                    </div>
                  </div>
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    <button onClick={() => setEditingPlanFor(null)} className="text-xs text-gray-500 hover:text-gray-700 ml-auto">Cancel</button>
                  </div>
                ) : (
                  <button onClick={() => setEditingPlanFor(item.id)} className="text-sm text-brand-700 hover:text-brand-900">+ Add Dave Care protection</button>
                )}
              </div>
            )}
          </div>
      ...
      <aside className="card p-5 h-fit sticky top-20">
        ...
        {remaining > 0 && (<p className="mt-4 text-xs text-brand-700 bg-brand-50 border border-brand-100 rounded-md p-2">Add {money(remaining)} more for free shipping.</p>)}
